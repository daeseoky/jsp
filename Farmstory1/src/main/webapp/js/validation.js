...
//Validation 수행(유효성검증)
//데이터 형식을 검사하기 위한 정규 표현식(Regular Expression)
...
$(function(){
			...
			success: function(data){
				...
				if(data.result > 0){
					...
					if(reUid.test(uid)){
						...
						$('.resultId').css('color', 'red').text('아이디는 영문 소문자,숫자조합 4~10자까지 입니다');
						isUidOk = false;
					}
				}
			}
	...
	$('input[name=pass2]').focusout(function(){
		...
		if(pw1 == pw2){
			...
			if(rePass.test(pw2)){
				...
				$('.resultPass').css('color', 'red').text('비밀번호는 영문 숫자 특수문자 조합 최소 4자리 이상 이어야 합니다.');
				isPassOk = false;
			}
		}else{
	...
	$('input[name=name]').focusout(function(){
		...
		if(reName.test(name)){
			...
			$('.resultName').css('color', 'red').text('이름은 한글로 2자이상이어야합니다.');
			isNameOk = false;
		}
	});
		...
		$.get('/Farmstory1/user/proc/checkNick.jsp', jsonData, function(data){
			...
			if(data.result > 0){
				...
				if(reNick.test(nick)){
					...
					$('.resultNick').css('color', 'red').text('별명은 영문 소문자, 숫자, 한글 조합 최소 2~5자리 이상 이어야 합니다.');
					isNickOk = false;
				}
			}
